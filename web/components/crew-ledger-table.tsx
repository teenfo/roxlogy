    ...
    <span className="relative shrink-0">
      ...
      {open && (
        <>
          ...
          />
          <span className="absolute right-0 top-8 z-20 flex w-[160px] flex-col rounded-[10px] border border-line-strong bg-inset p-1.5 shadow-[0_12px_30px_rgba(0,0,0,.5)]">
            {children}
          </span>
        </>
      )}
...
export function CrewLedgerTable({
  ...
  const seg = (k: typeof kind, label: string, n: number) => (
    <button
      ...
      className={`inline-flex h-7 items-center gap-1.5 rounded-full px-3 text-[13px] font-bold transition-colors ${
        kind === k ? "bg-accent text-on-accent" : "text-foreground/80 hover:text-foreground"
      }`}
    >
      {label}
      <span className={`text-[11px] ${kind === k ? "text-accent-ink" : "text-muted"}`}>{n}</span>
    </button>
  );
  ...
  return (
    <div className="overflow-hidden rounded-[14px] border border-line bg-card">
            ...
            <ul className="mt-3 flex max-h-[55vh] flex-col gap-1.5 overflow-y-auto">
              {openGroup.rows.map((r) => (
                ...
                >
                  ...
                  {r.memo && (
                    <span className="w-full truncate text-[11px] text-muted">{r.memo}</span>
                  )}
                </li>
              ))}
            </ul>
      ...
      {items.map((it) =>
        it.type === "group" ? (
          ...
          >
            <span className="flex shrink-0 items-baseline gap-1 sm:flex-col sm:gap-0">
              ...
              <span className="text-xs text-muted-2">{weekday(it.date)}</span>
            </span>
            <span className="min-w-0">
              <span className="block truncate text-sm font-semibold">{groupTitle(it)}</span>
              <span className="mt-0.5 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted">
                <span
            ...
            <span className="flex shrink-0 items-center gap-1.5 sm:contents">
              ...
              <span className="text-right sm:block">
                ...
                <span className="block text-[11px] text-muted">{t("crew.finGroupNoBalance")}</span>
              </span>
              <span aria-hidden className="hidden text-right text-[11px] text-muted-2 sm:block">
                ›
              </span>
            </span>
          </button>
        ...
        <div key={it.row.id} className={`${cols} border-b border-line-soft px-[18px] py-2.5 hover:bg-card-hover`}>
          <span className="flex shrink-0 items-baseline gap-1 sm:flex-col sm:gap-0">
            ...
            <span className="text-xs text-muted-2">{weekday(it.row.entry_date)}</span>
          </span>
          ...
          <span className="flex shrink-0 items-center gap-1.5 sm:contents">
            <span className="text-right sm:block">
              ...
              <span className="tabular block text-[11px] text-muted">{won(it.row.balance)}</span>
            </span>
            {isStaff && !closed ? (
      ...
      {shown.length === 0 && (
        <p className="px-[18px] py-10 text-center text-[13px] text-muted-2">
          {t(rows.length ? "crew.finFilterEmpty" : "crew.finEmpty")}
        </p>
      )}

      {rows.length > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-line bg-inset px-[18px] py-3 text-xs text-muted">
          <span>
