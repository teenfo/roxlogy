...
const COL = "text-[11px] font-bold tracking-[0.06em] text-muted";
    ...
    <span className="relative shrink-0">
      ...
      {open && (
        <>
          ...
          />
          <span className="absolute left-0 top-9 z-20 flex gap-1.5 rounded-[10px] border border-line-strong bg-inset p-2 shadow-[0_12px_30px_rgba(0,0,0,.5)]">
            {TIER_COLORS.map((c) => (
              <button
                key={c}
                type="button"
...
export function CrewTierManage({
  ...
  const cellLabel = "text-[11px] font-bold text-muted lg:hidden";
    ...
    <div className="flex flex-col gap-3.5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className="h-[38px] shrink-0 rounded-lg bg-accent px-4 text-sm font-extrabold text-on-accent hover:brightness-110"
        >
          + {t("crew.tierAdd")}
        </button>
      </div>
      ...
      {adding && (
        <form onSubmit={add} className={`${CARD} flex flex-wrap items-center gap-2.5 p-[18px]`}>
          ...
          <button
            ...
            className="h-10 rounded-lg bg-accent px-4 text-sm font-extrabold text-on-accent disabled:opacity-40"
          >
            {t("crew.tierAdd")}
          </button>
          <button
      ...
      <div className={CARD}>
            ...
            >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-2 pl-[38px] lg:contents">
                <span className="flex items-center gap-1.5 lg:justify-center">
                  ...
                  <button
                    ...
                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 ${
                      x.is_default ? "border-accent" : "border-line-strong hover:border-muted"
                    }`}
                  >
                ...
                <button
                  ...
                  onClick={() => remove(x)}
                  className="ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-sm text-danger hover:bg-danger-card disabled:cursor-not-allowed disabled:text-muted-3 disabled:hover:bg-transparent lg:ml-0 lg:justify-self-center"
                >
                  ×
                </button>
              </div>
        ...
        <p className="flex flex-wrap gap-x-4 gap-y-1 px-[18px] pb-3 text-xs text-muted">
