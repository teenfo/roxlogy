...
export default async function FeedPage({
  ...
  const tabCls = (active: boolean) =>
    `rounded-full border px-3 py-1 text-xs ${active ? "border-accent text-accent-ink" : "border-muted/40 text-muted hover:border-foreground"}`;

  return (
    <main className="rx-page rx-list-page rx-feed-page">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-2xl font-bold">{t("feed.title")}</h1>
        <Link
          href="/members"
          className="rounded-md border border-accent/50 px-3 py-1.5 text-sm font-semibold text-accent-ink hover:bg-accent/10"
        >
          {t("feed.findMembers")}
        </Link>
      </div>
          ...
          {feed.map((r) => (
            ...
            >
              ...
              <RowLink
                ...
                className="font-mono text-lg font-semibold text-accent-ink hover:underline"
              >
