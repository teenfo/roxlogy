...
3. **S12 공식 결과 수집 검토 — 컷오버 이후** — 스크래핑 정책, 저작권, 선수 개인정보 처리는 **컷오버 이후**로 미룸. 출시 전에는 본인 등록(S7) + 익명 집계 분포(S13 v1)로 시작
...
| Android 패키지 | applicationId `app.roxlogy.android` 폰·워치 공통 (Data Layer 요건 — 2026-07-26), 네임스페이스는 android/wear 분리 |
...
> 2026-09-20 전체 화면 리뉴얼: 마크·워드마크·브랜드 색상은 이 절의 원본을 유지한다. 웹 앱의 작업 표면은 밝은 배경·흰 패널로 변경하며, 라이브보드·공유물은 다크를 유지한다. 현재 UI 규칙은 `ROXLOGY-Global-Design-Spec.md`와 `ROXLOGY-Mobile-Design-Guide.md`를 따른다.
...
- 옐로는 아이덴티티·액션에만 사용한다. 브랜드 공유물·라이브보드의 대면적 배경은 다크, 웹 앱의 작업 표면은 리뉴얼 스펙의 밝은 배경이다. 옐로 배경은 아이콘·배지·주요 액션 등 소면적 한정.
