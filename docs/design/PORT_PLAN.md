...
| 5 | 사이드바 푸터 "내 크루" 가 여러 개일 때 | **확정(2026-09-20): 사용자가 고정할 크루를 고른다.** `profiles.sidebar_crew_id`(마이그레이션 1건, 본인만 갱신) + 설정 화면에서 선택. 미선택이면 가장 오래 가입한 크루 |
| 6 | `/design-coverage` 갤러리 | **가져오지 않음**(디자인 검수용) |

**2026-09-20 승인.** 1·2·3·4·6 은 추천대로, 5 는 위와 같이 확정. P0 부터 진행.
