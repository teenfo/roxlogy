...
## 종료(closed)는 완전한 잠금이다
서버가 11개 경로를 모두 `race_closed` 로 거부한다(2026-09-12 검증):
참가·시작·스플릿·마지막 취소·초기화(본인), 스태프의 참가자 추가·웨이브 출발·스플릿·취소·초기화·제거.
고치려면 **다시 열어야** 한다.

화면도 같은 상태를 그대로 비춘다.
- 스태프 화면: 참가자 추가·웨이브 출발 카드를 그리지 않고, 카드마다 완료 버튼·취소·초기화·제거 대신
  "종료된 레이스입니다" 한 줄만 둔다. 남는 버튼은 "다시 열기" 하나다(비활성이 아니라 DOM 에서 제거 —
  숨기기만 하면 키보드로 눌린다).
- 완주하지 못한 참가자는 **미완주(DNF)**: 경과 시계를 멈추고 강조를 걷는다.
  종료된 레이스에서 시계가 계속 흐르면 아직 측정 중인 것처럼 읽힌다. 보드도 같다
  (지표의 "측정 중"이 "미완주"로, 진행 바의 현재 칸 표시도 멈춘다).
- 참가자 화면은 **완주한 뒤에도 폴링을 멈추지 않는다**(15초 간격). 멈추면 그 사이 레이스가 종료돼도
  이 화면만 모르고 "완주 취소" 버튼을 계속 내준다(서버는 거부하므로 헛손질이 된다).

완주한 참가자의 카드는 스태프 화면에 **그대로 남는다** — 순서도 참가 순서 그대로다.

## 레이스 찾기 (마이그레이션 095)
코드를 못 받은 사람이 많아서, 참가는 "코드 입력"이 아니라 **고르기**로 시작한다.
- `/pft/race/join` — `pft_race_joinable()` 이 주는 "지금 참가할 수 있는 레이스"를 먼저 보여 주고 버튼으로 참가한다.
  대상은 **진행 중 + 코드 참가 허용** 레이스 중 내 크루 것이거나 크루가 없는 것. 남의 크루 행사는 목록에 넣지 않는다
  (코드를 받았으면 아래 코드 입력으로 참가). 이미 참가한 레이스는 "참가 중"으로 표시하고 내 화면으로 보낸다.
- `/admin/races` — `pft_race_admin_list()`. 전체 관리자만. 누가 만들었든 모든 레이스를 진행 중/지난으로 나눠
  크루·만든 사람·참가/완주 수·코드와 함께 표로 본다. 보드·스태프로 바로 간다.
- 레이스 행은 보드 때문에 RLS 가 전체 공개라 이 두 함수는 노출을 늘리지 않는다. 목록 범위만 좁힌다.
- `/pft` 허브의 레이스 카드는 화면 **맨 위**(헤더 바로 아래)로 올렸다 — 맨 아래에 있으면 눈에 띄지 않는다.
