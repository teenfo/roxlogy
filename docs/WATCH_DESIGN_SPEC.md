# Roxlogy 워치 UI 디자인 스펙 (v0.5.0 기준 + v0.6.0 변경 요약)
...
## v0.6.0 변경 요약 (클로드 디자인 시안 반영)

본문 5절의 v0.5.0 레이아웃 중 아래가 시안대로 교체됐다. 상세 수치는 코드가 기준.

- **시뮬 진행 화면(RUN·STATION·록스존)**: T자 3분할 → **5밴드 그리드** (헤어라인 1dp `#2A2A2A` 구분).
  ① `TOTAL │ 목표 대비` (목표 대비 셀은 Good/Bad 로 전체 채움·검정 글자, 폭 0.46w)
  ② 구간 정체성 한 줄 — `RUN n · 0.42 / 1.00 km` / `STATION n · SkiErg` (머신 스테이션은 이 줄이 PM5 연결 버튼, 연결되면 `✓ · 230W 32spm`) / `록스존 · 이동`
  ③ `심박 19sp │ 구간시간 32sp │ 목표 19sp` (시안 44sp 를 원형 수직 공간에 맞춰 32sp 로 조정, 폭 0.64w)
  ④ `다음 구간` + `NEXT UP · 다음 목표` ⑤ 주 액션 (0.55w×42dp)
- **IDLE**: 목표 있으면 `목표 총시간(옐로) │ 1km 랩` 미리보기 밴드 추가.
- **일시정지**: 구간 한 줄 + 구간시간 32sp + `TOTAL │ 목표 대비` 밴드.
- **DONE/SENT**: 총시간 26sp + 목표 대비 채움 뱃지 + `런(블루)│스테이션(옐로)│록스존` 합계 밴드 + `♥ 평균·최고`.
- **주 액션 통일**: 전 화면 42dp 높이·15sp Bold (시안 196×42 대응. 3영역 내 0.9w / 그리드 0.55w / 리스트 화면 0.86w).
- **에르그 흐름 확장**(`ErgScreen.kt`): 기기 선택 리스트(↑SkiErg/↔RowErg/◷기기 없이 시간만) → 스캔(중지 가능) → 실패 시 주변 PM5 목록(이름·RSSI·기억함, 탭=연결) → 연결 대기(0m/0W/0spm 3열 + 핸들 당기면 자동 시작) → 기록(**거리 40sp 주 지표** + 시간/출력·spm/페이스 3열 밴드) → 완료(평균 출력·평균 /500m) → 전송됨.
