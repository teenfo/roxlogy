...
3. 앱에서 로그인 → **설정 → 알림 → "앱 알림 켜기"** → 권한 허용 → **"테스트 발송"** 클릭 → 폰 알림 도착 확인.
4. 실전 트리거: 다른 계정으로 팔로우(즉시~1분 내 "새 팔로워"), 또는 WOD 리마인더 시각 설정(해당 시각 ±5분).

> (선택) Firebase 프로비저닝 후 GitHub 리포 변수 `ROXLOGY_REQUIRE_FCM=true` 를 설정하면,
> 이후 릴리스에서 google-services.json 이 빠졌을 때 FCM 없는 APK 가 조용히 게시되지 않고 빌드가 실패한다.
