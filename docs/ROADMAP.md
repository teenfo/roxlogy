...
> 다음: **M6 워치 앱** (상표 정밀검색 게이트 + 네이티브 빌드 환경 필요) 또는
> **S13 백분위**(법적 검토 게이트). 웹으로 완결 가능한 Phase 2(S8·S9·S10)는 완료.
...
2. **S13 백분위 랭킹** ✅ (v1 공개 분포 기반) — `race_benchmarks`(디비전×성별
   ...
   **잔여 게이트**: 실제 공식 결과 대량 수집(정밀 분포 교체)은 여전히 법적 검토 필요.
   (2026-07-08)
...
- [ ] S13 공식 결과 **대량 수집** 법적 검토 — 정밀 분포로 교체 시 (v1 공개 분포는 완료)
