...
- **RPC 의 반환 모양을 바꾸는 변경은 배포와 원자적이지 않다.** 마이그레이션은 즉시
  ...
  바꾸자 구 번들이 `undefined.length` 로 3분간 500). 컬럼 이름·타입을 바꾸거나
  없앨 때는 **덧붙였다가 나중에 걷어내기**(새 컬럼 추가 → 배포 → 다음 마이그레이션에서
  옛 컬럼 제거)를 쓰거나, 최소한 **배포가 READY 된 뒤에 마이그레이션을 적용**할 것.
  컬럼 추가·함수 신설처럼 옛 코드가 무시하고 지나가는 변경은 순서를 신경 쓰지 않아도 된다.
